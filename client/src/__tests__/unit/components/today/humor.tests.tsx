...
describe('/ Page Unit Test', () => {
  test('유머 게시글이 불러와지는지', async () => {
    ...
    const findDescriptionByTestId = await screen.findByTestId('humor-description');

    const expectedTitle = within(findTitleByTestId).getByText(expectedMockData.title);
    const expectedDescription = within(findDescriptionByTestId).getByText(expectedMockData.description);
